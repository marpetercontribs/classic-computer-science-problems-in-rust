    ...
    fn successors(&self) -> Vec<MCState> {
        ...
        if self.boat_on_west_bank {
            if self.west_missionaries > 1 {
                successors.push(MCState::new(self.west_missionaries-2, self.west_cannibals, !self.boat_on_west_bank));
            }
            if self.west_missionaries > 0 {
                successors.push(MCState::new(self.west_missionaries-1, self.west_cannibals, !self.boat_on_west_bank));
            }
            if self.west_cannibals > 1 {
                successors.push(MCState::new(self.west_missionaries, self.west_cannibals-2, !self.boat_on_west_bank));
            }
            if self.west_cannibals > 0 {
                successors.push(MCState::new(self.west_missionaries, self.west_cannibals-1, !self.boat_on_west_bank));
            }
            if self.west_missionaries > 0 && self.west_cannibals > 0 {
                successors.push(MCState::new(self.west_missionaries-1, self.west_cannibals-1, !self.boat_on_west_bank));             
            }
        } else {
...
impl ToString for MCState {
    fn to_string(&self) -> String {
        let mut result = format!("On the west bank there are {} missionaries and {} cannibals.\n", self.west_missionaries, self.west_cannibals);
        result.push_str(&format!("On the east bank there are {} missionaries and {} cannibals.\n", self.east_missionaries, self.east_cannibals));
        result.push_str(&format!("The boat is on the {} bank.\n", if self.boat_on_west_bank {"west"} else {"east"}));
        result
    }
}
...
fn display_solution(path: &Vec<MCState>) {
    ...
    for current_state in &path[1..] {
        if current_state.boat_on_west_bank {
            println!("{} missionaries and {} cannibals moved from the east bank to the west bank.",
                old_state.east_missionaries - current_state.east_missionaries,
                old_state.east_cannibals - current_state.east_cannibals);
        } else {
            println!("{} missionaries and {} cannibals moved from the west bank to the east bank.",
...
fn main() {
    ...
    let solution = generic_search::bfs(start, |&state| state.goal_test(), |&state| state.successors() );
    match solution {
