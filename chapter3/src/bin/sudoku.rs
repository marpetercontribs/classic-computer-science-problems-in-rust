...
impl csp::Constraint<CellLocation,usize> for SudokuConstraint {
    fn satisfied(&self, assignment: &HashMap<CellLocation,usize>) -> bool {
        for cell_location in assignment.keys() {
            ...
            for checked_location in assignment.keys().filter(
                |loc| &cell_location != loc && value == assignment.get(loc) ) {
                ...
                if subgrid_row == checked_subgrid_row &&
                   subgrid_column == checked_subgrid_column {
                    return false;
                }                    
            }  
        }
...
fn main() {
    ...
    for (row_index, row) in puzzle.grid.iter().enumerate() {
        for (column_index, column) in row.iter().enumerate() {
            let cell_location = CellLocation { row: row_index, column: column_index};
    ...
    let mut csp = csp::CSP::<CellLocation,usize,SudokuConstraint>::new(locations.clone(), domains);
